...
export default function App() {
  ...
  const results = useMemo(() => {
    const costPerBookshelf = (3 * paperPrice) + (1 * leatherPrice) + (1.5 * logPrice);
    
    let bookshelfCount = 0;
    if (mode === 'investment') {
      bookshelfCount = Math.floor(inputValue / costPerBookshelf);
    } else if (mode === 'stack') {
    ...
    const totalCost = bookshelfCount * costPerBookshelf;
    ...
    return {
      ...
      profitMargin,
      breakdown: {
        paper: bookshelfCount * 3,
        leather: bookshelfCount * 1,
        logs: bookshelfCount * 1.5,
      }
    };
